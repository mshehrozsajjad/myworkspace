...
<div class="wrapper">
          ...
          <ul class="treeview-menu">
            <?php
              $sql = "select * from department d where departmentname in (select dept_name from department d, user_dept du where du.dept_name=d.departmentname and du.user_name=d.`user-name` and du.dept_id=d.iddepartment and du.`user-name`='$userName') and d.project_id in (select du.project_id from department d, user_dept du where du.dept_name=d.departmentname and du.user_name=d.`user-name` and du.dept_id=d.iddepartment and du.project_id='$idprojects');";
                    ...
                    while($rws = mysql_fetch_array($result)){ 
                    ?>
            <li><a href="./department.php?projectid=<?php echo $rws['iddepartment'] ?>"><i class="fa fa-circle-o"></i> <?php  echo  $rws['departmentname']?></a></li>
              <?php 
                    } 
              ?>
          </ul>
  ...
  <div class="content-wrapper">
    ...
    <img src="images/myoffice.jpg" class="img-rounded img-responsive" alt="Dashboard" >
  ...
  <div class="row">
      ...
      <div class="col-md-12 personal-info">
        ...
        <form class="form-horizontal" role="form" method='post' action="completeDepartment.php?projectid=<?php echo $idprojects ?>">
         
         <div class="form-group">
            <label class="control-label col-sm-3" ><span class="glyphicon glyphicon-user"></span> Add Users</label>
            <div class="col-md-6">
            <select name="username[]"class="selectpicker" data-live-search="true" multiple   data-width="100%" data-selected-text-format="count > 2"data-size="4"  title="Search by Name Or Username" >
                     <?php
                $sql ="select * from websiteusers";
                 $result = mysql_query($sql) or die(mysql_error());
                    while($rws = mysql_fetch_array($result) ){
                        if($rws['username'] != $userName){ 
                 ?>
              ...
              <p class="text-danger">Select as many users you want to.<br/> Alternativley You can add users later too.</p>          
              </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-md-6">
             <textarea class="form-control" rows="5" id="comment" name="departmentdescription" required placeholder="Details About Your Department"></textarea>
            </div>
          </div>
          <div class="form-group">
