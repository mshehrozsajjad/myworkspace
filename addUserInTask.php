    ...
    {    
            $selectedUsername = mysql_real_escape_string((strip_tags($selectedUsername)));
             ...
             $query = "INSERT INTO user_dept (`user-name`,`dept_id`,dept_name,user_name,can_edit,project_id) VALUES ('$selectedUsername','$iddepartment','$departmentname','$deptuser','0','$idprojects')";	        
              $data = @mysql_query ($query); 
              $query = "SELECT username,taskname FROM myworkspace.tasks where idtasks = '$idtasks';";
             $data = mysql_query ($query)or die(mysql_error());
             if (mysql_num_rows($data)==1){ 
                $row = mysql_fetch_array($data);
             ...
             $query = "INSERT INTO user_task (`username`,`dept_id`,taskid,taskname,user_name,can_edit,project_id) VALUES ('$taskuser','$iddepartment','$idtasks','$taskname','$selectedUsername','0','$idprojects')";
	         $data = mysql_query ($query)or die(mysql_error());   
